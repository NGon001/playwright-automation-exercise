...
test.describe("API Product & Catalog tests", () => {
    test('GET /api/brandsList returns valid brand list', async ({ productsAPI }) => {
        const expectedCode = Status.success;
        await productsAPI.brandsList(Methods.GET,expectedCode);
    });

    test('POST, PUT, DELETE (/api/brandsList) Incorrect req method', async({ productsAPI }) => {
    ...
    test('POST /api/searchProduct returns products for valid keywords', async ({ productsAPI }) => {
        const expectedCode = Status.success;
    ...
    test('GET, PUT, DELETE (/api/searchProduct) Incorrect req method', async({ productsAPI }) => {
        const expectedCode = Status.methodNotAllowed;
        const expectedMessage = "This request method is not supported.";
        for (const method of Object.values(Methods)) {
    ...
    test('POST /api/searchProduct returns empty result for non-existent product', async ({ productsAPI }) => {
        const expectedCode = Status.success;
        await productsAPI.searchProduct(Methods.POST,expectedCode,undefined,"top1231");
    });

    test('POST /api/searchProduct returns 400 Bad Request when keyword is missing', async ({ productsAPI }) => {
